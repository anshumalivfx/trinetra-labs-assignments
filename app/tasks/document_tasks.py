...
def log_execution(db, job_id: int, level: str, step: str, message: str, metadata: Dict = None):
...
def process_document_task(
    ...
    with get_db_context() as db:
        try:
            ...
            log_execution(
                db, job.id, "INFO", "document_loaded",
                f"Document loaded: {document.filename}"
            )
            
            # Extract PDF content
            logger.info("extracting_pdf_content", job_id=job_id, document_id=document_id)
            ...
            for stage_name, stage_data in stages.items():
                if stage_name in ["pdf_analysis", "email_composition", "email_validation"]:
            ...
            log_execution(
                db, job.id, "INFO", "agents_completed",
            ...
            email_record = EmailRecord(
                ...
                status=EmailStatus.SENT.value if email_delivery.get("success") else EmailStatus.FAILED.value,
                ...
                error_message=email_delivery.get("error") if not email_delivery.get("success") else None,
            )
            db.add(email_record)
            db.commit()
            
            # Update job with final status
            job.status = JobStatus.COMPLETED.value if orchestration_result.get("success") else JobStatus.FAILED.value
